...
/**********************************************************************/
void SelectionSort(int sortdata[], int left, int right) {
  int i;
  int k;
  ...
  for (i = left; i < right; i++) {
    int indexMin = i;
    int temp = 0;
    for (k = i+1; k < right+1; k++) if (sortdata[k] < sortdata[indexMin]) indexMin = k;
    temp = sortdata[i];
    sortdata[i] = sortdata[indexMin];
    sortdata[indexMin] = temp;
  }
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 1) {
    printf("%s\n", L_NAME);
    printf("usage: ./selection [data_init type]\n");
  ...
  start = get_time();
  SelectionSort(data, 0, DATANUM-1);
  end = get_time();
